...
interface ImportCategoryContentDesktopProps {
  ledgerId: string
  onCancel: () => void
}

export function ImportCategoryContentDesktop({
  ledgerId,
  onCancel,
}: ImportCategoryContentDesktopProps) {
  const {
    file,
    parsing,
    validating,
          ...
          {showWarning && usedCategories.length > 0 ? (
            <div className="mb-4 rounded-lg bg-yellow-50 p-4 dark:bg-yellow-950">
              ...
              <p className="mb-2 text-xs text-yellow-700 dark:text-yellow-300">
                다음 카테고리가 기존 거래내역에서 사용 중입니다. 덮어쓰기하면
                새 거래 추가 시 사용할 수 없게 됩니다.
              </p>
